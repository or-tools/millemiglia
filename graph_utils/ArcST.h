...
protected:
	...
	*/
	...
	/**
	*	cost = arc cost; if this is the first arc of a line-rotation, the cost is equal to the entire cost, otherwise it is zero
	*/
	double cost;

	/**
	*	travelling_time = time to travel the arc, if type = "travelling", otherwise it is zero
	*/
	int travelling_time;
...
public:
	 ...
	 * \param cost
	 */
	ArcST(const int& departure_id, const int& arrival_id, const string& type, const string& line, const string& rotation, const int& travelling_time = 0.0, const double& cost = 0.0);
	/**
