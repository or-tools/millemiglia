...
class ElRandom {
 public:
  ...
  /** Generates a random float according to the Lomax distribution, whose
  density function is X = lambda * ((1/(1-U)^(1/alpha)) - 1),
  *     where U is a uniformly distributed random variable in [0,1)
  ...
  /** Generates a random integer according to the discrete distribution
   * according to the weights W in parameter P(i|W_0,...,W_{n-1}) =
   * W_i/sum_{k=0,...,n-1} W_k, for each i=0,...,n-1; n is the size of W
   *   \param weights
   */
  template <typename T>
  ...
  /** Generates a random integer according to the binomial distribution
   * according to probability of success p in the range [0,t] P(i|t,p) = (t i)
   * p^i(1-p)^{t-i}
   *   \param p
   *   \param t
   */
  ...
  static unsigned int GetSeed() { return GetInstance().seed; }

  static std::mt19937& GetGenerator() { return GetInstance().g; }
